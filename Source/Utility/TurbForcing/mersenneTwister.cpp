...
unsigned long mt19937::init_seed;
unsigned long mt19937::mt[mt19937::N];
int mt19937::mti;
...
//
void
mt19937::sgenrand(unsigned long seed)
{
...
void
mt19937::reset(unsigned long seed)
{
  sgenrand(seed);
}
...
void
mt19937::save(amrex::Vector<unsigned long>& state) const
{
  state.resize(N + 2);
  state[0] = init_seed;
  for (int i = 0; i < N; i++) {
...
int
mt19937::RNGstatesize() const
{
  return N + 2;
}

void
mt19937::restore(const amrex::Vector<unsigned long>& state)
{
  if (state.size() != N + 2) {
...
void
ResetRandomSeed(unsigned long seed)
{
...
void
SaveRandomState(amrex::Vector<unsigned long>& state)
{
...
void
RestoreRandomState(const amrex::Vector<unsigned long>& state)
{
...
void
UniqueRandomSubset(
  amrex::Vector<int>& uSet, int setSize, int poolSize, bool printSet)
{
  if (setSize > poolSize) {
